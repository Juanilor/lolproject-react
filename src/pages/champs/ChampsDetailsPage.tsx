...
interface Champ {
  ...
  title?: string;
  image?: any;
  skins?: Array<SkinObject>
  lore?: string;
  spells?: Array<SpellsObject>
};
...
export const ChampsDetailsPage = () => {
  ...
  useEffect(() => {
    if (params.id) {
      let id = params.id
      ChampsService.getOne(params.id).then(data => setChamp(data.data[id]));
    }
  }, [])
  ...
  return (
    <MenuWrapper>
      ...
      <p>{params.id}</p>
      {
        champ && champ.name && champ.lore && champ.spells && champ.passive &&
        <div className='champ-data-container'>
          <img className='champ-screen' src={`http://ddragon.leagueoflegends.com/cdn/img/champion/loading/${champ.id}_0.jpg`} />
          <div className='champ-data'>
            <p className='champ-title'>{champ.name}</p>
            <p className='champ-lore'>{champ.lore}</p>
            <div className='champ-spells'>
              {champ.spells.map(spell => (
                <div className='spell'>
                  <img src={`http://ddragon.leagueoflegends.com/cdn/13.8.1/img/spell/${spell.id}.png`} />
                  <p className='spell-title'>{spell.name}</p>
                  <p>{spell.description}</p>

                </div>
              ))}
              <div className='spell'>
                <img src={`http://ddragon.leagueoflegends.com/cdn/13.8.1/img/passive/${champ.passive.image.full}`} />
                <p className='spell-title'>{champ.passive.name}</p>
      ...
      <div className='champ-skins-container'>
        {
          champ.skins &&
          champ.skins.map((skin: SkinObject) => (
            <div className='champ-skin'>
              <img src={`http://ddragon.leagueoflegends.com/cdn/img/champion/loading/${champ.id}_${skin.num}.jpg`} />
              <p className='champ-skin-title '>{skin.name === "default" ? '' : skin.name}</p>
            </div>
          ))}
      </div>
    </MenuWrapper>
